...
class World:
	...
	# Note - do not return entities marked as dead
        #return (x for x in self._entities if x not in self._dead_entities)
        return list(set(self._entities) - set(self._dead_entities))

    def entities_with_component(self, component_type):
        """Return a list of entities that have given component"""
        try:
